...
Traffic_Papers 是一个基于 Obsidian 的**网络流量安全研究论文知识库**，当前收录 92 篇顶级会议/期刊论文（其中 46 篇已完成深度分析），服务于综述写作、项目申报、方法比较和实验复现。
...
├─────────────────────────────────────────────────────────┤
│  第二层：论文笔记层（03）                                  │
│  92 篇结构化中文笔记，每篇 14–57 KB（40 篇含深度分析）     │
├─────────────────────────────────────────────────────────┤
│  第一层：原始资料层（00–02）                               │
...
| 目录 | 内容 | 当前数量 | 快速定位场景 |
|------|------|----------|-------------|
| `00-inbox/PDFs/` | 论文 PDF 原文 | 92 篇 | 查原文、定位引用 |
| `01-mineru-output/` | MinerU 原始解析（已 gitignore） | 92 份 | 调试解析问题 |
| `02-parsed-markdown/` | 整理后的 MinerU Markdown | 92 份 | 机器可读原文 |
| `03-paper-notes/` | **结构化论文笔记**（核心） | 92 篇（40 篇深度分析） | 论文分析、证据追踪 |
| `04-concepts/` | 概念页 | 9 个 | 理解领域概念 |
| `05-methods/` | 方法页 | 8 个 | 方法对比、技术选型 |
| `06-tasks/` | 任务页 | 8 个 | 任务定义、输入输出 |
| `07-surveys/` | 综述页 | 5 个 | 综述写作素材 |
| `08-comparisons/` | 对比表（含开源注册表） | 5 个 | 横向比较、代码追踪 |
| `09-claims/` | 观点与矛盾 | 2 个（33 Claims + 13 矛盾） | 论证、找 research gap |
...
## 5. 46 篇深度分析论文（L3/L4 级）

这些论文已完成公式推导、消融实验、跨论文关联分析（400–800 行/篇）：

### 第一批（2026-05-27，14 篇）

| 论文 | 会议 | 主题 |
|------|------|------|
| SoK: Decoding the Enigma | S&P 2025 | 12 种分类器系统化评估 |
| The Sweet Danger of Sugar | SIGCOMM 2025 | 揭示表征学习评估漏洞 |
| MM4flow | CCS 2025 | 多模态预训练流量模型 |
| Training with Only 1.0‰ | CCS 2025 | 极端少样本 + 跨模态融合 |
...
| RF (Subverting WF) | USENIX 2023 | 突破网站指纹防御 |
| Proxy Fingerprinting | USENIX 2024 | TLS 握手指纹识别 |
| FEC-OSL | TIFS 2026 | 开放集半监督分类 |

### 第二批（2026-06-09，26 篇 CCF A 级）

| 论文 | 会议 | 主题 |
|------|------|------|
| Deep Fingerprinting | CCS 2018 | CNN 网站指纹攻击 |
| Whisper | CCS 2021 | 频域恶意流量检测 |
| Point Cloud Malicious Detection | CCS 2023 | 点云分析恶意流量 |
| Tunneled Flooding Detection | CCS 2024 | 隧道洪泛流量检测 |
| Training Robust Classifiers | CCS 2025 | MAML 元学习动态条件 |
| Robust Multi-tab WF | S&P 2023 | 多标签网站指纹攻击 |
| TrafficFormer | S&P 2025 | 高效预训练流量模型 |
| Unknown Malicious Traffic | NDSS 2023 | 流量交互图未知恶意检测 |
| Fine-grained Webpage Fingerprinting | NDSS 2024 | 大规模细粒度 WF |
| Low-quality Training Data Detection | NDSS 2024 | 低质量数据鲁棒检测 |
| Black-Box Evasion Attack | NDSS 2026 | 黑盒逃逸攻击 |
| CertTA | USENIX 2025 | 认证鲁棒性流量分析 |
| FlowPic | INFOCOM 2019 | 图像化流量分类 |
| MIETT | AAAI 2025 | 多实例加密流量 Transformer |
| FlowRefiner | NeurIPS 2025 | 抗标签噪声分类框架 |
| Wedjat | KDD 2025 | 因果分析逃逸攻击检测 |
| Traffic-Explainer | KDD 2026 | 流量分类可解释性框架 |
| DecETT | WWW 2025 | 加密隧道应用指纹 |
| DApp GNN | TIFS 2021 | GNN 去中心化应用识别 |
| FG-SAT | TIFS 2025 | 流量图环境迁移分类 |
| ASNet | TIFS 2025 | 无需预训练的 SOTA 分类 |
| FEC-OSL | TIFS 2026 | 开放集半监督分类 |
| MT-DEGCL | TIFS 2026 | 多任务双嵌入图对比学习 |
| LEXNet | SIGKDD 2023 | 轻量可解释 CNN 分类 |

### 第三批（2026-06-10，6 篇 arXiv）

| 论文 | 年份 | 主题 |
|------|------|------|
| NetMamba | arXiv 2024 | 单向 Mamba 预训练流量分类 |
| TrafficGPT | arXiv 2024 | GPT-style 自回归流量分类+生成 |
| STAR | arXiv 2025 | 零样本 HTTPS 网站指纹 |
| NetMamba+ | arXiv 2026 | Mamba+Flash Attention+多模态+OOD |
| TrafficMoE | arXiv 2026 | 异质性 MoE 加密流量分类 |
| Bias in the Shadows | arXiv 2026 | 捷径学习检测与评估批判 |

**跨论文关联链**：
- **预训练演进线**：ET-BERT → YaTC → MM4flow → TrafficFormer → MIETT → Sweet Danger → SoK
- **SSM 演进线**：NetMamba → NetMamba+（参数量仅 Transformer 1/85，性能接近）
- **MoE 架构线**：TrafficMoE（异质路由） ↔ DAIR-FedMoE（联邦 MoE）
- **无需预训练线**：ASNet（WSA+CSS 领域适配） vs 预训练阵营
- **恶意检测对比线**：SmartDetector ↔ tFusion (1‰ Samples) ↔ Whisper ↔ Point Cloud
- **WF 攻防演进链**：Deep Fingerprinting → RF → Swallow（攻击） ↔ Palette（防御） ↔ STAR（零样本） ↔ Robust Multi-tab WF ↔ Fine-grained WF
- **逃逸攻击线**：Black-Box Evasion ↔ Wedjat（因果分析） ↔ CertTA（认证鲁棒性）
- **评估批判三角**：SoK（虚假繁荣） ↔ Sweet Danger（评估泄漏） ↔ Bias in the Shadows（捷径学习）
- **流量生成线**：TrafficGPT（分类+生成） ↔ Talk Like a Packet（基础模型）

---

## 6. 33 条核心 Claims（可直接引用）
...
| DF vs Palette | WF 攻防 | DF 声称攻破防御，Palette 证明新防御可抵御 |
| ET-BERT vs Sweet Danger | 数据集划分 | per-packet split 导致数据泄漏，per-flow split 下 F1 从 96.8% 降至 6.7% |
| MM4flow vs Sweet Danger | 表征学习价值 | TB 级预训练突破 vs 正确评估下浅层模型仍占优 |
| 多数论文 vs Sweet Danger | 评估指标 | micro F1 偏向多数类，掩盖少数类性能，差距可达 30%+ |
| SoK vs ET-BERT | 信息来源本质 | SoK 证明仅头部信息有效，ET-BERT 声称 payload 字节模式可学 |
| Palette vs Swallow | 防御鲁棒性 | Palette 声称可抵御 SOTA 攻击，Swallow 证明自适应攻击仍可部分攻破 |
| ASNet vs 预训练阵营 | 预训练必要性 | ASNet 无预训练达 SOTA（WSA+CSS），ET-BERT 去预训练 F1 降 37.57% |
...
| 类别 | 数量 | 说明 |
|------|------|------|
| 已确认开源（本库论文） | 30 | 含 GitHub/GitLab 地址，覆盖 6 个研究方向 |
...
| 问题类型 | 严重程度 | 数量 | 状态 |
|----------|----------|------|------|
| 断裂 wikilinks | 高 | ~300 → 0 条 | **已全部修复**（2026-06-09） |
| 旧格式 frontmatter | 高 | 13 篇 | **已修复**（2026-06-03） |
| 缺少 PDF/MinerU 路径 | 高 | 9 篇 | **已修复**（2026-06-03） |
| 概念/方法页面不足 | 中 | 13 个待创建 | 按需创建 |
| 实验章节薄弱 | 中 | 2 篇 | **已修复**（2026-06-03） |

**剩余工作**：
1. 创建 13 个待扩展概念/方法页面（meta-learning, attention-mechanism, autoencoder 等）— 列表已转为纯文本，待创建后恢复为 wikilink

---

## 9. 论文入库流程（17 步）

> 详细规范见 AGENTS.md。推荐使用一键入库管道 `templates/claudian-prompts/06-ingest-pipeline.md` 自动执行全部步骤。
...
每篇论文笔记包含 14 个章节（深度分析论文扩展至 §3.4/§3.5/§13/§14）：
...
| §3.4 问题发现路径 | 深度分析论文：4 行表格追溯研究起点 |
| §3.5 科学假设形成 | 深度分析论文：假设验证设计 |
...
| §13 写作叙事分析 | 深度分析：5 小节（叙事结构/论证策略/图表设计/写作质量/借鉴价值） |
| §14 跨论文关联 | 深度分析：与相关论文的具体关联点 |
...
| Claudian Prompts | 8 种标准化工作流 | `templates/claudian-prompts/00–07` |
...
| 2026-06-01 | 研究动机链与叙事分析结构迭代：新增 §3.4/§3.5/§13 模板，动机/叙事对比表 |
| 2026-06-03 | 全库质量检查：修复 frontmatter/pdf/mineru_md/断链/重复文件，对比表增至 5 个 |
| 2026-06-09 | 13 篇新论文入库（papers_temp），知识库总量增至 92 篇，开源注册表增至 30 个 |
| 2026-06-09 | 26 篇 CCF A 级论文深度分析（14→40 篇），Claims 增至 42 条，矛盾增至 9 组 |
| 2026-06-10 | 6 篇 arXiv 论文深度分析（40→46 篇），新增 SSM/MoE/零样本 WF/捷径学习方向 |
...
**断链处理**：概念页和方法页中的 wikilinks 如指向不存在的页面，应转为纯文本（不保留 `[[]]` 语法），避免产生断裂链接。待对应页面创建后再恢复为 wikilink。详见 AGENTS.md §7。

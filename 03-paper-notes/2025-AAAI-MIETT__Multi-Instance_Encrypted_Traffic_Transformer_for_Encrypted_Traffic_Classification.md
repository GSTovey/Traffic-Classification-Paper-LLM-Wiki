...
| 辅助假设 | PRPP 和 FCL 预训练任务能增强模型对时序和 flow 动态的理解 | packet 顺序和 flow 内聚性是关键信号 | 消融实验（Table 3） |

**假设验证结果**：

| 假设 | 支撑/反驳 | 关键实验证据 | 位置 |
|---|---|---|---|
| 核心假设 | 支撑 | 移除 flow attention 后 CrossPlatform(Android) 错误率上升 16.4% | §Ablation Study, Table 4 |
| 辅助假设 | 支撑 | 添加 PRPP 和 FCL 后 F1 从 73.62% 提升至 82.36% | §Ablation Study, Table 3 |
...
| Step 2 | 匿名化 packet | 十六进制转换，bi-gram tokenize，BPE 编码 | Token 序列 (max 128) | 数值化 |
| Step 3 | Token 序列 | Position embedding + Value embedding | Packet representation | 向量化 |
| Step 4 | 多个 packet representation | 堆叠为矩阵 X ∈ R^(N×L×d) | Flow representation | 多实例表示 |
| Step 5 | Flow representation | TLA Layer: Packet Attention → transpose → Flow Attention × M 层 | 增强的 flow representation | 层次化建模 |
| Step 6 | [CLS] tokens | 预训练任务或 fine-tuning 分类 | 分类结果 | 任务适配 |
...
**Two-Level Attention (TLA)**:
- Packet Attention: 对每个 packet 内部的 token 做 MHSA，复杂度 O(L²d) per packet
- Flow Attention: 对每个 position 跨 packet 做 MHSA，复杂度 O(N²d) per position
- 总复杂度: O(NL²d + LN²d)，比扁平化方案 O(N²L²d) 高效约 4.8 倍（L=128, N=5）

**PRPP**: 对 packet pair (i,j) 预测相对位置，使用交叉熵损失

**FCL**: 使用 cosine similarity 构建对比学习，同 flow 内不同 position 的 packet 为正对，不同 flow 的为负对

**总预训练损失**: L_pt = L_MFP + α·L_PRPP + β·L_FCL
...
| 方法 | 优点 | 缺点 | 本文改进点 |
|---|---|---|---|
| ET-BERT | 开创性 datagram 表示学习 | 仅考虑相邻 packet 关系 | 引入 Flow Attention 建模全局 packet 关系 |
| YaTC | 将流量视为图像进行 patch 化 | 扁平化丢失时序信息 | 保留 packet 结构，用 TLA 建模层次关系 |
| Deep Packet | CNN 自动特征提取 | 严重依赖标注数据 | 预训练 + fine-tuning 范式 |
...
CrossPlatform (Android) 数据集上，MIETT 相比 ET-BERT 在 Accuracy 上提升 8.27%，F1 上提升 14.66%，在 212 类应用分类的高类别数场景下优势显著。
...
1. 使用 ET-BERT 预训练的 packet attention checkpoint
2. 在 5 个数据集的训练集上进行无标签预训练（150K steps）
3. 在各下游任务上有标签微调（30 epochs）

### 7.3 关键超参数、预训练和训练细节

- Packet length: 128 tokens
- Packet 数量: 5
- Embedding dim: 768
- TLA 层数: 12
- 学习率: 2×10⁻⁵
- MFP masking ratio: 15%
- PRPP/FCL 权重: α=β=0.2
- Optimizer: AdamW

### 7.4 能否迁移到其他任务？

是。Multi-Instance 的思路和 TLA 结构可迁移到其他需要建模层次结构的序列数据任务中。PRPP 和 FCL 预训练任务也可应用于其他 flow 级别分析任务。

### 7.5 对我的研究有什么启发？

1. **多实例学习视角**：将 flow 视为 packet 的 bag 是建模流量层次结构的有效方式
2. **分层注意力**：分别建模 intra-packet 和 inter-packet 关系比扁平化更高效且更有效
3. **专用预训练任务**：针对流量数据特点设计预训练任务（而非直接借用 NLP/CV 任务）能带来显著提升
